...
const LaunchKitView: React.FC<LaunchKitViewProps> = ({ kit }) => {
  return (
    <div className="p-10 max-w-5xl mx-auto space-y-10 pb-32">
      <div className="mb-6">
        <h2 className="text-3xl font-black text-white">Sosyal Medya Lansman Kiti</h2>
        <p className="text-slate-400">Kitabınızın tanıtımı için hazırlanan otonom içerikler.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="glass p-8 rounded-3xl border border-white/10 shadow-sm">
          ...
          <div className="space-y-4">
            {kit.instagram.map((post, i) => (
              <div key={i} className="bg-white/5 p-4 rounded-2xl text-sm text-slate-300 leading-relaxed border border-white/10">
                {post}
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-2 bg-black/40 text-white p-8 rounded-3xl shadow-xl border border-white/5">
          <h3 className="font-bold text-amber-400 mb-4 flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
            Lansman E-postası
          </h3>
          <pre className="whitespace-pre-wrap font-sans text-sm text-slate-300 leading-relaxed bg-black/30 p-6 rounded-2xl border border-white/10">
            {kit.email}
          </pre>
        </div>
      </div>
